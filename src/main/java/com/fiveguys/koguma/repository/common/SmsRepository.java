...
public class SmsRepository {
    ...
    private final int LIMIT_TIME = 3000 * 60; // 인증번호 유효 시간

    private final StringRedisTemplate stringRedisTemplate;

    // Redis에 저장
    public void addAuthNum(SmsDTO smsDTO) {
        System.out.println(smsDTO.getAuthNumber());
        System.out.println(smsDTO.getTo());
        stringRedisTemplate.opsForValue()
                .set(PREFIX + smsDTO.getTo(), smsDTO.getAuthNumber(), Duration.ofSeconds(LIMIT_TIME));
    }
